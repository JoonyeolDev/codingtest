...
# 대결은 준비된 음식들을 일렬로 배치한 뒤, 한 선수는 제일 왼쪽에 있는 
# 음식부터 오른쪽으로, 다른 선수는 제일 오른쪽에 있는 음식부터 왼쪽으로 
# 순서대로 먹는 방식으로 진행됩니다. 중앙에는 물을 배치하고, 
...
# 음식의 배치는 "1223330333221"이 됩니다. 따라서 1번 음식 1개는 
# 대회에 사용하지 못합니다.

# 수웅이가 준비한 음식의 양을 칼로리가 적은 순서대로 나타내는 
# 정수 배열 food가 주어졌을 때, 대회를 위한 음식의 배치를 나타내는 문자열을 
...
for idx,value in enumerate(food):
    num_str += str(idx)*(value//2)
answer = num_str+'0'+num_str[::-1]
print(answer)
