...
def solution(k, tangerine):
    ...
    sorted_count = sorted(tangerine_count.items(), key=lambda x: x[1], reverse=True)
    answer = 0
    for _, count in sorted_count:
        k -= count
        answer += 1
        if k <= 0:
            return answer
...
def solution(k, tangerine):
    ...
    for tan in tangerine:
        tangerine_dict[tan] += 1
    sorted_dict = sorted(tangerine_dict, key=lambda x: tangerine_dict[x], reverse=True)
