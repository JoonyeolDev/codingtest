...
def solution(want, number, discount):
    ...
    len_dis = len(discount) - 9
    for idx in range(len_dis):
...
# 1차 수정 : 로직 수정
def check_discount(want_dict, lst):
    copy_dict = want_dict.copy()
    for item in lst:
        if copy_dict.get(item, 0):
            copy_dict[item] -= 1
        else: return 0
    return 1
def solution(want, number, discount):
    ...
    answer = 0
    idx = 0
    len_dis = len(discount) - 10
    while idx <= len_dis:
        if check_discount(want_dict, discount[idx:idx+10]):
            cnt = 1
            for i in range(idx,len_dis):
...
# 초기 코드
def check_discount(want, number, lst):
    want_dict = {key: value for key, value in zip(want,number)}
    for item in lst:
        if want_dict.get(item, 0):
            want_dict[item] -= 1
        else: return 0
    return 1
def solution(want, number, discount):
